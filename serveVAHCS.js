...
vahcsdb_client.connect((res,err)=>{
    ...
    switchboxConnection = vahcsdb_client.db("internalData").collection("SWITCHBOX");
    switchboxConnection.findOne({name:"switchbox_linkArray"},(error, data)=>{            //Change this to your database, collection, and index name
        if(data == null){
            console.log("-<>-(!) count not fetch switchbox array!");
        }else{
            console.log("-<>-fetched switchbox array!");
            console.log(eval(data.data));
            switchboxArray = eval(data.data);
        }
    });
});
...
app.get("/30", function(req, res){       //Keller Free Stuff Table Unlisted Object Request
    ...
    vahcsdb_client.db("SNIFFS").collection("TYPE_2_SNIFFS").insertOne({"source":"Junk Vault Europa Cabinet 2 QR Code","timestamp":buildTimestamp2()});
});
...
var nullTokens = [
    ...
    ["Ian's Seconday Levento Chromebook 2 localhost","1UQyvMuCFkgG"],
];
...
app.get("/JRARASS_claimAlert", function(req, res){
    ...
    if(combinedTokens.indexOf(queryObject.ip) == -1){ 
        ...
        vahcsdb_client.db("SNIFFS").collection("TYPE_1_SNIFFS").insertOne({"source":"JRARASS unfulfilled claim","token":queryObject.ip,"timestamp":buildTimestamp2()});
    }
});
app.get("/JRARASS_claimAlert_claimFulfilled", function(req, res){
    ...
    if(combinedTokens.indexOf(queryObject.ip) == -1){ 
        ...
        vahcsdb_client.db("Claimed_JRARASS_Objects").collection("CJO_PartIndexes").insertOne({"part_index":queryObject.partData, "claim_timestamp":buildTimestamp(),"token": queryObject.ip});
        vahcsdb_client.db("SNIFFS").collection("TYPE_1_SNIFFS").insertOne({"source":"JRARASS Fullfilled Claim","token":queryObject.ip,"timestamp":buildTimestamp2()});
    }
});
app.get("/JRARASS_claimAlert_claimAborted", function(req, res){
    ...
    if(combinedTokens.indexOf(queryObject.ip) == -1){ 
        ...
        vahcsdb_client.db("SNIFFS").collection("TYPE_1_SNIFFS").insertOne({"source":"JRARASS aborted claim","token":queryObject.ip,"timestamp":buildTimestamp2()});
    }
});
...
app.get("/switchbox", function(req, res){
    var queryObject = url.parse(req.url,true).query;
    console.log("Switchbox query object: "+JSON.stringify(queryObject)+" <<->> index: "+queryObject.n);
    if(queryObject.n != undefined){
        console.log("User requesting to be redirected");
        if(queryObject.n < switchboxArray.length){
            res.redirect(switchboxArray[queryObject.n][0]);
            sendEmailToElliot("QR "+queryObject.n+" scanned", "someone went to "+switchboxArray[queryObject.n][1]+" from a QR code");
            vahcsdb_client.db("SNIFFS").collection("TYPE_2_SNIFFS").insertOne({"source":switchboxArray[queryObject.n][1],"timestamp":buildTimestamp2()});
            console.log("User redirected sucsessfully");
        }else{
            console.log("User requesting to be redirected to an array index that does not exist!");
            res.json({error: "index does not exist in switchboxArray"});
        }
    }else{
        console.log("User requesting switchbox homepage");
        res.sendFile(__dirname+"/switchbox.html");
    }
});

app.post("/switchbox", function(req, res){
    console.log("Incomming Post from /switchbox.html, command: "+req.body.command+"   data: "+req.body.data);
    if(req.body.command == "generateQR"){
        switchboxArray.push(req.body.data);
        switchboxConnection.updateOne({name:"switchbox_linkArray"},{$set:{data: JSON.stringify(switchboxArray)}},(error, data)=>{
            encodeAsQR("vahcs-server.herokuapp.com/switchbox?n="+(switchboxArray.length-1),res);
        });
    }else{
        res.download("./qr_result.png");
    }
});

function encodeAsQR(desitination, res){
    QRCode.toDataURL(desitination, function (err, code) {
        if(err) return console.log("error occurred")
        console.log(code);
        let filePath = "qr_result.png";
        imageDataURI.outputFile(code, filePath).then(()=>{res.send("done!")});
    });
}
